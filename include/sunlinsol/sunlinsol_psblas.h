...
struct _SUNLinearSolverContent_PSBLAS {
  ...
  amg_c_dprec *mh;             /* MLD2P4 preconditioner                     */
...
#define LS_CCTXT_P(S) ( PSBLAS_CONTENT(S)->cctxt )
 ...
 * --------------------------------------- */
SUNDIALS_EXPORT SUNLinearSolver SUNLinSol_PSBLAS(psb_c_SolverOptions options, char methd[], char ptype[], psb_c_ctxt *cctxt);
/* Core Functions */
...
SUNDIALS_EXPORT int SUNLinSolSetScalingVectors_PSBLAS(SUNLinearSolver S, N_Vector s1,
						      N_Vector s2);
/* Get functions */
