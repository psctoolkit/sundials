...
SUNMatrix SUNPSBLASMatrix(psb_c_ctxt *cctxt, psb_c_descriptor *cdh)
{
  SUNMatrix A;
  SUNMatrix_Ops ops;
...
SUNMatrix SUNMatClone_PSBLAS(SUNMatrix A)
{
  ...
  ops->matvec      = SUNMatMatvec_PSBLAS;
  ops->space       = SUNMatSpace_PSBLAS;
	ops->matvecsetup = NULL; // PSBLAS doesn't need setup before Matvec
...
int SUNMatScaleAddI_PSBLAS(sunrealtype c, SUNMatrix A)
{
  /* matrix should be in the ASSEMBLED state */
  if(!psb_c_dis_matasb(SM_PMAT_P(A),SM_DESCRIPTOR_P(A)))
    psb_c_dspasb(SM_PMAT_P(A),SM_DESCRIPTOR_P(A));
...
int SUNMatScaleAdd_PSBLAS(sunrealtype c, SUNMatrix A, SUNMatrix B)
{
