...
SUNLinearSolver SUNLinSol_PSBLAS(psb_c_SolverOptions options, char methd[], char ptype[], psb_c_ctxt *cctxt){
  ...
  the other routine, here we decide only if we are using a PSBLAS or an AMG4PSBLAS
...
int SUNLinSolSetup_PSBLAS(SUNLinearSolver S, SUNMatrix A){
  ...
  if( !psb_c_cmp_ctxt(*(LS_CCTXT_P(S)),*(SM_CCTXT_P(A)))) {
    psb_c_get_i_ctxt(*(LS_CCTXT_P(S)),&ictxt1,&ret);
    psb_c_get_i_ctxt(*(LS_CCTXT_P(S)),&ictxt2,&ret);
...
sunrealtype SUNLinSolResNorm_PSBLAS(SUNLinearSolver S){
  if (S == NULL) return(-ONE);
...
/* ---------------------------------------
 * Interfaces for PSBLAS/AMG4PSBLAS routine for
...
int SUNLinSolSeti_PSBLAS(SUNLinearSolver S, const char *what, psb_i_t val){
  ...
  return(amg_c_dprecseti(LS_MLPREC_P(S), what, val));
}
int SUNLinSolSetc_PSBLAS(SUNLinearSolver S, const char *what, const char *val){
  if (S == NULL || LS_MLPREC_P(S) == NULL) return(-1);
  return(amg_c_dprecsetc(LS_MLPREC_P(S), what, val));
}
